...
from prompts import sql_generation_prompt, sql_response_prompt
...
with st.sidebar:
    ...
    password = st.text_input("Password", type="password", value="root123")
...
def getDatabaseSchema():
    return st.session_state.db.get_table_info() if "db" in st.session_state else "Please connect to a database."
...
def runQuery(query):
    return st.session_state.db.run(query) if "db" in st.session_state else "Please connect to database"

def getResponseForQueryResult(question, query, result):
    chain = sql_response_prompt | llm
    response = chain.invoke({
        "question": question,
        "schema": getDatabaseSchema(),
        "query": query,
...
if connectBtn:
    with st.spinner("Connecting to database..."):
        ...
        st.chat_message("assistant").markdown("Hello! I'm your SQL assistant. Ask me anything about your database.")
